...
if [[ -n "${COLLIDED:-}" ]]; then
  ...
  echo "          (currently: ${COMPOSE_SUBNET}.)"
  echo "          The 'down' is NOT optional. A bare 'up -d' recreates the network"
  echo "          but REATTACHES the containers it did not rebuild without their"
  echo "          service-name aliases, so the subnet looks right, everything is"
  echo "          attached and healthy, and 'postgres' stops resolving. Measured."
  echo "          scripts/upgrade.sh does the down/up for you when the value changes."
  echo ""
...
if (( QUICK )); then
  ...
  if [[ -z "$DNS_NET" || -z "$DNS_TARGET" ]]; then
    ...
    if (( EDNS_RC == 0 )); then
      ...
      echo "                    echo 'COMPOSE_SUBNET=<free /24>' >> .env"
      echo "                    docker compose down && docker compose up -d"
      echo "                  (the 'down' matters: a bare 'up -d' leaves the"
      echo "                   containers it did not rebuild attached WITHOUT"
      echo "                   their service-name aliases, which looks fine and"
      echo "                   resolves nothing. collect-diagnostics.sh flags it.)"
