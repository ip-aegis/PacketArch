...
def _load_codeline():
    ...
    candidates = [
        here / "codeline.py",  # sibling copy (portable deploy, e.g. alpha)
        Path(__file__).resolve().parents[2]
        / "backend/app/protocol_engines/atcs/codeline.py",  # in-repo
    ]
    for f in candidates:
        if f.exists():
...
def frame_stream(args):
    ...
    for n in itertools.count():
        ...
        if args.control_every and n and n % args.control_every == 0:
            ...
            yield frame, f"indication {wayside}->office"
...
def main() -> None:
    ...
    p.add_argument("--prefix", default="#",
                   help="record type prefix ('#'=ATCS; '' for none)")
