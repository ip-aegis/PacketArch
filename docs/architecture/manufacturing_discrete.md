# Reference Architecture: Manufacturing Discrete
...
### Archetype: `manufacturing_discrete_cell` — Discrete Manufacturing — Cell-Based

Multi-cell discrete-manufacturing plant. Each cell has a PLC + HMI + drives + I/O on a vendor-native fieldbus (PROFINET, EtherNet/IP, or mixed). L3 SCADA polls all cells; L3.5 IDMZ brokers to IT. Standard pattern for automotive assembly, discrete electronics, packaging, food&beverage discrete lines.

- **Pattern**: `discrete_cell`
- **Default vendor profile**: `siemens_shop`
- **Supported vendor profiles**: `siemens_shop`, `rockwell_shop`, `multi_vendor`
- **Min scale**: `demo`
...
|---|---|---|---|
| `idmz` (Industrial DMZ) | L3.5 | critical | `jump_server`, `remote_access_gateway`, `patch_staging_server`, `av_management_server`, `historian_replica`, `opc_ua_aggregator`, `wan_edge_router`, `core_switch` |
| `operations` (Operations) | L3.0 | high | `scada_primary`, `scada_standby`, `process_historian`, `engineering_workstation`, `nms_server`, `asset_management_server`, `mes_server`, `core_switch` |
| `cell1` (Production Cell 1) | L1.5 | standard | `cell_controller`, `safety_controller`, `area_hmi`, `vfd`, `servo`, `distributed_io`, `cell_switch` |
| `cell2` (Production Cell 2) | L1.5 | standard | `cell_controller`, `safety_controller`, `area_hmi`, `vfd`, `servo`, `distributed_io`, `cell_switch` |
| `cell3` (Production Cell 3) | L1.5 | standard | `cell_controller`, `safety_controller`, `area_hmi`, `vfd`, `servo`, `distributed_io`, `cell_switch` |
| `cell4` (Production Cell 4) | L1.5 | standard | `cell_controller`, `safety_controller`, `area_hmi`, `vfd`, `servo`, `distributed_io`, `cell_switch` |

#### Conduits (allowed cross-zone protocols)

| Conduit | Direction | Allowed protocols |
|---|---|---|
| `idmz` ↔ `operations` | bidirectional | `https`, `snmp`, `opc_ua`, `rdp`, `ssh`, `smb` |
| `operations` ↔ `cell1` | bidirectional | `ethernet_ip`, `modbus_tcp`, `s7comm`, `profinet`, `opc_ua`, `snmp` |
| `operations` ↔ `cell2` | bidirectional | `ethernet_ip`, `modbus_tcp`, `s7comm`, `profinet`, `opc_ua`, `snmp` |
| `operations` ↔ `cell3` | bidirectional | `ethernet_ip`, `modbus_tcp`, `s7comm`, `profinet`, `opc_ua`, `snmp` |
| `operations` ↔ `cell4` | bidirectional | `ethernet_ip`, `modbus_tcp`, `s7comm`, `profinet`, `opc_ua`, `snmp` |

#### Notes

- Cell counts: DEMO=1, SMALL=2, MEDIUM=3, LARGE/MULTI_SITE=4.
- Safety PLC included only at LARGE+ (smaller plants typically embed safety functions in the cell PLC).
- MULTI_VENDOR vendor profile spreads cells across siemens/rockwell/schneider/abb. Comm matrix routes cross-vendor flows over OPC UA.

## Role catalog (28 roles applicable)
...
| `area_supervisor_plc` | L2.0 | area | snmp | Optional. Include only at enterprise-scale discrete manufacturing where cells need a coordinator. |
...
| `servo` | L0.0 | process | snmp | Include in discrete manufacturing motion applications: CNC, robotic cells, packaging. |
| `discrete_sensor` | L0.0 | process | - | Optional; usually represented at the I/O block level rather than as standalone devices. |
...
## Communication matrix (38 entries)
...
|---|---|---|---|---|---|
| `scada_primary` | `cell_controller` | poll | 1000 | `opc_ua` | manufacturing_discrete |
| `process_historian` | `cell_controller` | subscription | 5000 | `opc_ua` | manufacturing_discrete |
| `engineering_workstation` | `cell_controller` | configuration | 60000 | `opc_ua` | manufacturing_discrete |
| `engineering_workstation` | `safety_controller` | configuration | 120000 | `opc_ua` | manufacturing_discrete |
| `area_hmi` | `cell_controller` | poll | 500 | `opc_ua` | manufacturing_discrete |
| `cell_controller` | `distributed_io` | cyclic_io | 4 | `modbus_tcp` | manufacturing_discrete |
| `cell_controller` | `vfd` | cyclic_io | 8 | `modbus_tcp` | manufacturing_discrete |
| `cell_controller` | `servo` | cyclic_io | 2 | `ethernet_ip` | manufacturing_discrete |
| `safety_controller` | `cell_controller` | safety | 4 | `modbus_tcp` | manufacturing_discrete |
| `safety_controller` | `vfd` | safety | 4 | `modbus_tcp` | manufacturing_discrete |
| `safety_controller` | `distributed_io` | safety | 4 | `modbus_tcp` | manufacturing_discrete |
