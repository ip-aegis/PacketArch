# Reference Architecture: Distribution Logistics
...
### Archetype: `distribution_warehouse` — Distribution / Logistics — Warehouse

Fulfillment center / sortation hub / cold-chain warehouse. WMS at L3 issues pick / wave / putaway tasks; per-zone WCS PLCs drive conveyor and sortation lanes; conveyor PLCs run tight motion to drives + sensors. Default vendor is Rockwell (EtherNet/IP-dominant) but Siemens / Schneider / multi-vendor supported.

- **Pattern**: `warehouse_pick`
- **Default vendor profile**: `rockwell_shop`
- **Supported vendor profiles**: `rockwell_shop`, `siemens_shop`, `schneider_shop`, `multi_vendor`
- **Min scale**: `demo`
...
|---|---|---|---|
| `idmz` (Industrial DMZ) | L3.5 | critical | `jump_server`, `reverse_proxy`, `patch_staging_server`, `historian_replica`, `wan_edge_router`, `core_switch` |
| `operations` (WMS / WCS Operations) | L3.0 | high | `scada_primary`, `scada_standby`, `process_historian`, `engineering_workstation`, `mes_server`, `nms_server`, `core_switch` |
| `zone1` (Warehouse Zone 1) | L1.0 | standard | `wcs_controller`, `conveyor_controller`, `vfd`, `distributed_io`, `area_hmi`, `cell_switch` |
| `zone2` (Warehouse Zone 2) | L1.0 | standard | `wcs_controller`, `conveyor_controller`, `vfd`, `distributed_io`, `area_hmi`, `cell_switch` |
| `zone3` (Warehouse Zone 3) | L1.0 | standard | `wcs_controller`, `conveyor_controller`, `vfd`, `distributed_io`, `area_hmi`, `cell_switch` |
| `zone4` (Warehouse Zone 4) | L1.0 | standard | `wcs_controller`, `conveyor_controller`, `vfd`, `distributed_io`, `area_hmi`, `cell_switch` |
...
| `operations` ↔ `zone1` | bidirectional | `ethernet_ip`, `modbus_tcp`, `snmp` |
| `operations` ↔ `zone2` | bidirectional | `ethernet_ip`, `modbus_tcp`, `snmp` |
| `operations` ↔ `zone3` | bidirectional | `ethernet_ip`, `modbus_tcp`, `snmp` |
| `operations` ↔ `zone4` | bidirectional | `ethernet_ip`, `modbus_tcp`, `snmp` |
...
| `wcs_controller` | L1.0 | control | snmp | Required in distribution_logistics scenarios. |
| `conveyor_controller` | L1.0 | control | snmp | Required in distribution_logistics for fulfillment / parcel-sorting scenarios. |
...
|---|---|---|---|---|---|
| `scada_primary` | `wcs_controller` | poll | 1000 | `ethernet_ip`, `modbus_tcp`, `opc_ua` | distribution_logistics |
| `process_historian` | `wcs_controller` | subscription | 5000 | `opc_ua`, `ethernet_ip` | distribution_logistics |
| `engineering_workstation` | `wcs_controller` | configuration | 120000 | `ethernet_ip`, `modbus_tcp`, `opc_ua` | distribution_logistics |
| `wcs_controller` | `conveyor_controller` | poll | 100 | `ethernet_ip`, `modbus_tcp` | distribution_logistics |
| `conveyor_controller` | `vfd` | cyclic_io | 8 | `ethernet_ip` | distribution_logistics |
| `conveyor_controller` | `distributed_io` | cyclic_io | 4 | `ethernet_ip` | distribution_logistics |
| `area_hmi` | `wcs_controller` | poll | 500 | `ethernet_ip`, `modbus_tcp` | distribution_logistics |
| `mes_server` | `process_historian` | subscription | 10000 | `opc_ua`, `https` | distribution_logistics |
