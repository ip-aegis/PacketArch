...
export function buildCommandRegistry(deps: RegistryDeps): CommandDefinition[] {
  ...
  const navItems: Array<{ path: string; label: string; icon: React.ReactNode; keywords?: string[] }> = [
    ...
    { path: '/libraries', label: 'Go to Libraries', icon: e(DatabaseOutlined), keywords: ['reference', 'cve', 'attack', 'device'] },
    { path: '/libraries?tab=devices', label: 'Go to Device Library', icon: e(DatabaseOutlined), keywords: ['templates', 'profiles', 'fingerprints', 'signatures'] },
    { path: '/libraries?tab=cves', label: 'Go to CVE Browser', icon: e(BugOutlined), keywords: ['vulnerabilities', 'security'] },
    { path: '/libraries?tab=attacks', label: 'Go to Attack Library', icon: e(AppstoreOutlined), keywords: ['playbook', 'kill chain', 'mitre'] },

    { path: '/live-traffic?tab=deployments', label: 'Go to Deployments', icon: e(CloudServerOutlined), keywords: ['running', 'active'] },
