...
- **Server**: `<SERVER_IP>` (this machine)
- **User**: `<SSH_USER>`
- **URL**: `https://<SERVER_IP>`
- **Credentials**: Set via `ADMIN_PASSWORD` in `.env`
...
Production `.env` file (generated by `scripts/server-init.sh`):
```
...
|------------|---------|----------|-------|
| *(configure per environment)* | `<AGENT_HOST_IP>` | `<SSH_USER>` | SSH key installed |

**SSH Access:**
```bash
ssh <SSH_USER>@<AGENT_HOST_IP>
```
...
Supported contexts:
- `deployment` - Deployment configuration help
...
│       │   └── deployments.tsx # Deployment help
│       ├── pages/             # Route pages
│       │   ├── ScenariosPage.tsx
