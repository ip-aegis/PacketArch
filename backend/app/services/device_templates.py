...
def get_fingerprint_by_vendor_model(
    ...
    Uses vendor_fingerprints module as the SINGLE SOURCE OF TRUTH.
    Falls back to DEVICE_TEMPLATES for backward compatibility during transition.
    ...
    """
    # PRIMARY SOURCE: vendor_fingerprints module (authoritative)
    ...
    for fp in get_all_vendor_fingerprints():
        ...
        fp_model = fp.get("model", "").lower()

        if not vendors_match(vendor, fp_vendor):
            continue

        # Exact match
        if fp_model == model_lower:
            return fp.copy()

        # Partial match (handles spaces in model numbers like "6ES7 155" vs "6ES7155")
        model_normalized = model_lower.replace(" ", "")
        fp_model_normalized = fp_model.replace(" ", "")
        if model_normalized == fp_model_normalized:
            return fp.copy()

        # Flexible match for friendly names (e.g., "SINAMICS G120C" matches model_name)
        if model_lower in fp.get("vendor_family", "").lower():
            # Check if the model is the right variant
            if any(model_lower in str(v).lower() for v in fp.values() if isinstance(v, str)):
                return fp.copy()

    # FALLBACK: DEVICE_TEMPLATES (for backward compatibility)
...
def get_all_fingerprints() -> list[dict[str, Any]]:
    """Get all fingerprint dictionaries with complete protocol identities.

    Uses vendor_fingerprints module as the source of truth because it contains
    complete protocol identity data (modbus_identity, profinet_identity, etc.)
    ...
    """
    from app.services.vendor_fingerprints import get_all_vendor_fingerprints
    return get_all_vendor_fingerprints()
